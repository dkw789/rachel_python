...
class TestCheckSyllableCounts(unittest.TestCase):
    ...
    def test_multi_syllable_requirement_no_match(self):
        """Test check_syllable_counts with multiple syllable requirements. Since pattern[0] has 3 values, there must only be 3 lines in poem_lines. The number of words in each line is not being tested. The syllables in the lines of poem_lines do not match up with those in pattern[0]."""
        pattern = ([2, 1, 2], ['A', 'B', 'C'])
        word_to_phonemes = {'BEFORE': ['B', 'IH0', 'F', 'AO1', 'R'],
                            'NEXT': ['N', 'EH1', 'K', 'S', 'T'],
                            'POEM': ['P', 'OW1', 'AH0', 'M']}
